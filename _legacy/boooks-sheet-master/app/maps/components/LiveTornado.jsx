...
const LiveTornado = ({ latitude, longitude, showLabel, speed }) => {
  const [bookCount] = useState(500);

  // 🌀 speed-reactive rotation
  const rotationSpeed = Math.max(0.3, Math.min(2, speed ?? 0.5));

  const smoothRef = useRef();
  const velocity = useRef(new THREE.Vector3());
  const last = useRef({ lat: null, lng: null });

  // 🎛️ choose ONE decay value
  // 0.9 = soft / floaty
  // 0.85 = balanced (recommended)
  // 0.75 = snappy
  const DECAY = 0.85;

  /* ---------- Inject motion when GPS changes ---------- */
  useEffect(() => {
    if (last.current.lat !== null) {
      const strength = Math.min(2.0, (speed ?? 0.5) * 0.6 + 0.3);
    ...
    last.current = { lat: latitude, lng: longitude };
  }, [latitude, longitude, speed]);

  /* ---------- Smooth inertia every frame ---------- */
  useFrame(() => {
    if (!smoothRef.current) return;

    velocity.current.multiplyScalar(DECAY);
    smoothRef.current.position.add(velocity.current);
  });
  ...
  return (
    <Coordinates latitude={latitude} longitude={longitude}>
      <object3D ref={smoothRef}>
        {tornado}

        {showLabel && (
          <Billboard position={[0, 55, 0]}>
            {/* <Html scale={4} position={[0, 11, 0]} className="relative z-50">
               <svg
